					...
					<table border="0" cellspacing="0" cellpadding="10">
							<tr>
								...
								<select name="searchOption">
							      ...
							      <input class="button08" type="submit" value="검색">
							   </td>
							   </form>
							</tr>
							<tr class="board01">
							...
							<tr class="" align="center">
								<td  class="board02" style="color:darkolivegreen;">${bdto.bcode}</td>
								<%
								String id=(String)session.getAttribute("id");
								if(id.equals("admin")){
								%>
								<td  class="board03" >
									<a href="b_modifyView?bcode=${bdto.bcode }" style="color:darkolivegreen;">
									<c:choose>
										<c:when test="${fn:length(bdto.bname) > 18}">
											<c:out value="${fn:substring(bdto.bname,0,17) }" /> ...
										</c:when> 
										<c:otherwise>
											<c:out value="${bdto.bname}" />
										</c:otherwise>
									</c:choose>
									</a>
								</td>
								...
								<td  class="board02" style="color:darkolivegreen;">${bdto.bcategori}</td>
								<td  class="board02" style="color:darkolivegreen;">${bdto.bwriter }</td>
								...
								<td  class="board05">${bdto.brcount }</td>
